...
const char *const METHOD_NAME2 = "sub_numbers";
...
int main (int argc, char **argv)
{
	...
	if (dbus_message_is_method_call (message, INTERFACE_NAME, METHOD_NAME2)) {
            char *s;
            ...
            if (dbus_message_get_args (message, &dbus_error, DBUS_TYPE_STRING, &s, DBUS_TYPE_INVALID)) {
                ...
                if (!error) {
                    ...
                    sprintf (answer, "Difference is %d", i - j);
        ...
        else if (dbus_message_is_method_call (message, INTERFACE_NAME, METHOD_NAME)) {
            char *s;
            char *str1 = NULL, *str2 = NULL;
            const char space [4] = " \n\t";
            int i, j;
